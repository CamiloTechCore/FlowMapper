...
**Estado de esta actualización:** el archivo local `Code.gs` es **1.2.0**. La consulta remota más reciente a `getSchemaStatus` todavía devuelve **1.1.0** (un equipo, un proceso y ningún flujo). Debes publicar la versión nueva para guardar desde el editor. El diagnóstico de la versión antigua no conoce las columnas nuevas y por eso puede informar `requiereMigracion:false`. No se realizaron escrituras en Google Sheets desde esta tarea; las pruebas de guardado usan el `Code.gs` real con servicios GAS simulados.
...
La versión **1.2.0** mantiene `getData` e `insertRow` del script anterior, devuelve los registros creados y conserva los IDs suministrados (rechazando duplicados). `GET /exec` sin parámetros responde a `ping`; `getSchemaStatus` diagnostica la migración sin escribir. Consulta [las instrucciones de actualización](docs/ACTUALIZAR-APPS-SCRIPT.md). Las copias locales previas están en `docs/backups/`.
...
4. Añade cada nodo desde la paleta, por clic o arrastre. Selecciónalo para cambiar título, descripción, forma, configuración y parámetros. Inicio/fin usan óvalos, las actividades rectángulos, las decisiones rombos y los datos cilindros.
5. Arrastra de un puerto de salida a otro de entrada o usa **Conectar nodos**. Cada decisión requiere salidas **Sí** y **No**. Puedes crear bifurcaciones y retornos; un nodo de modelo se conecta mediante una línea discontinua al puerto de modelo de un nodo de IA.
6. Los botones **+ entrada / + salida**, **+ Parámetro** y **+ Adjunto** permiten ampliar cada elemento. Los adjuntos se describen como referencias; no se suben archivos ni se envían mensajes.
7. Arrastra el fondo para desplazarte, usa zoom, minimapa, **Ordenar** y **Ver todo**. Guarda para registrar el flujo en la **biblioteca**, agrupada por equipo y filtrable por proceso.
8. **Abrir mapa** muestra el diagrama 2D. Seleccionar un nodo centra la vista y abre sus propiedades. **Recorrer** permite elegir cada rama manualmente y regresar a pasos anteriores. Las dependencias de IA no se recorren como pasos.
9. **Editar flujo** reabre el mismo diagrama y guarda sobre el mismo flujo, sin duplicarlo. También conserva los identificadores de los nodos y conexiones existentes. Los datos de los flujos antiguos se leen con puertos predeterminados.
...
El alcance es **diseñar, guardar y recorrer**. Los bloques HTTP, correo, programación e IA guardan configuración descriptiva; no ejecutan automatizaciones, código, reglas o integraciones reales. Se permiten varios activadores y flujos que terminan en una acción sin un bloque Fin explícito. Se avisa de nodos sin ruta desde un activador y de IA sin modelo vinculado.
...
- `src/styles/workflow.css` y `workflow-shapes.css`: distribución del editor y figuras SVG 2D.
...
src/workflow/model.js          Tipos de nodo, puertos, conversión y validación del grafo

...
import { CATALOG, FLOW_STATES, normalizeStatus, isAnnotation, fromGraph } from '../../workflow/model';
...
export default function FlowMapPage({ flow, onBack }) {
  ...
  const trail = location.state?.flowTrail || [];
  const [activeId, setActiveId] = useState(() => new URLSearchParams(location.search).get('nodo')); 
  ...
  const activities = nodes.filter(n => !isAnnotation(n));
  const triggers = activities.filter(n => n.tipo === 'inicio');
  const referenced = flows.find(f => f.id === current?.refFlowId);
  const previousFlow = trail.at(-1);
  function goBack() {
    if (!previousFlow) { onBack(); return; }
    navigate('/flujos/' + encodeURIComponent(previousFlow.flowId) + '?nodo=' + encodeURIComponent(previousFlow.nodeId), { state: { flowTrail: trail.slice(0, -1) } });
  }
  function continueFlow() {
    if (referenced) navigate('/flujos/' + encodeURIComponent(referenced.id), { state: { flowTrail: [...trail, { flowId: flow.id, nodeId: current.id }].slice(-50) } });
    else if (outgoing.length === 1) follow(outgoing[0]);
    else if (!current) start();
  }
  function start() {
    const node = triggers.find(n => n.id === triggerId) || triggers[0] || activities.find(n => !edges.some(e => e.targetId === n.id)) || activities[0];
  ...
  return <div className="map-page">
    ...
    <div className="map-workspace">
      ...
      {current && <aside className="map-details" aria-label="Detalle del nodo"><div className="map-details-heading"><span>{config?.icon} {config?.label}</span>{!walking && <button aria-label="Cerrar detalle" onClick={() => setActiveId(null)}>×</button>}</div><h2>{current.titulo}</h2>
        ...
        {referenced && <button className="btn btn-secondary btn-sm" onClick={continueFlow}>Continuar a {referenced.nombre} →</button>}
        ...
        {!outgoing.length && !referenced && !isAnnotation(current) && <p className="map-terminal">✓ Fin del recorrido</p>}

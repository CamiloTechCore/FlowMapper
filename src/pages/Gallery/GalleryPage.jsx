...
export default function GalleryPage({ onOpenFlow }) {
  ...
  return <div className="library-page">
    ...
    {builderTeam && <FlowBuilder processes={processes.filter(p => p.teamId === builderTeam.id)} allFlows={flows} teamId={builderTeam.id} onClose={() => setBuilderTeam(null)} onSave={async (payload, options) => { const result = await saveFullFlow(payload); if (options?.background) return result; setBuilderTeam(null); setSavedId(result.flow.id); updateFilter(result.flow.teamId); setSearch(''); toast.success('Flujo guardado en la biblioteca'); return result; }} />}
  </div>;
}
